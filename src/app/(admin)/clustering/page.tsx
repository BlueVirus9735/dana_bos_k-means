...
interface KecResult {
  ...
  data: number[]; // [siswa, total_bos, sarpras, rombel, perpus, ibadah, toilet, uks, kl_baik, kl_rusak_ringan, kl_rusak_berat]
}
  ...
  const handleClustering = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setErrorMsg(
          data.error ||
            "Gagal melakukan clustering. Pastikan data kecamatan & BOS sudah diinput.",
        );
        return;
      }
      setResult(data);
        ...
        >
          ...
          <div>
            ...
            {loadingYears ? (
              ...
              >
                ...
                Belum ada data tahun ajaran. Masukkan data kecamatan &amp; BOS
                terlebih dahulu.
              </div>
            ) : (
          ...
          >
            {processing ? (
              ...
              <>
                <Play size={18} /> Jalankan Clustering K-Means —{" "}
                {tahunAjaran || "..."}
              </>
            )}
          </button>
        ...
        <div className="space-y-5 animate-in">
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                Tahun <strong>{result.tahun_ajaran}</strong> ·{" "}
                {result.results.n_clusters} cluster · {total} kecamatan diproses
            ...
            {distrib.map(({ key, items }) => {
              ...
              return (
                <div
                  key={key}
                  className="card"
                  style={{ padding: 20, border: `1px solid ${s.border}` }}
                >
                  <div
                    style={{
                      display: "flex",
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th title="Total Dana BOS">Total BOS</th>
                    <th title="Alokasi Dana Sarpras (20%)">Dana Sarpras</th>
                    <th title="Rombongan Belajar">Rombel</th>
                    <th title="Jumlah fasilitas: perpus+ibadah+toilet+uks">
                      Fasilitas
                    </th>
                    <th title="Kondisi Kelas: Baik / Rusak Ringan / Rusak Berat">
                      Kondisi Kelas (B/RR/RB)
                    </th>
                  </tr>
                </thead>
                <tbody>

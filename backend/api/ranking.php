...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Ambil nilai MAX sekali saja di luar loop (fix N+1 query problem)
    $max_query = "
        SELECT 
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Hitung priority score menggunakan MAX values yang sudah diambil di luar loop
        $siswa_score     = ($row['jumlah_siswa_total'] / $max_siswa) * 30;      // bobot 30%
        $ruang_score     = ($row['jumlah_ruang_kelas_total'] / $max_ruang) * 20; // bobot 20%
        ...
        $fasilitas_score = ($jumlah_fasilitas / $max_fasilitas) * 20;            // bobot 20%
        $dana_score      = ($row['total_dana_bos_total'] / $max_dana) * 30;      // bobot 30%
